...
const BlogEditor = () => {
  ...
  const fetchArticle = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.from('blog_articles').select('*').eq('id', articleId).single();
    ...
    try {
      ...
      for (const proxyUrl of proxies) {
        try {
          ...
          if (htmlContent && htmlContent.trim()) break;
        } catch {
          continue;
        }
      }
  ...
  const addListItem = (blockId: string) => {
    setBlocks(blocks.map((b) => (b.id === blockId ? { ...b, listItems: [...(b.listItems || []), ''] } : b)));
  };
  ...
  const applyFormat = (command: string, value?: string) => {
    ...
    if (element) updateBlock(selectedText.blockId, { content: element.innerHTML });
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (!el || !el.closest('.alignment-menu')) setShowAlignmentMenu(false);
    };
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      const el = e.target instanceof Element ? e.target : null;
      if (!el || !el.closest('.heading-menu')) setShowHeadingMenu(false);
    };
    document.addEventListener('mousedown', handleClickOutside);
    return () => document.removeEventListener('mousedown', handleClickOutside);
  }, [showHeadingMenu]);
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      const el = e.target instanceof Element ? e.target : null;
      if (!el || !el.closest('.heading-alignment-menu')) setShowHeadingAlignmentMenu(null);
    };
    document.addEventListener('mousedown', handleClickOutside);
    return () => document.removeEventListener('mousedown', handleClickOutside);
  ...
  const applyHeading = (headingType: 'h1' | 'h2' | 'p') => {
    ...
    document.execCommand('formatBlock', false, headingType === 'p' ? 'p' : headingType);
    const element = contentEditableRefs.current[selectedText.blockId];
    if (element) updateBlock(actualBlockId, { content: element.innerHTML });
  ...
  const TextToolbar = () => {
    ...
    return (
      ...
      >
        <button onClick={() => applyFormat('bold')} className="p-2 hover:bg-gray-100 rounded transition-colors" title="太字">
        ...
        <div className="relative heading-menu">
          ...
          {showHeadingMenu && (
            <div className="absolute top-full left-0 mt-1 bg-white rounded-lg shadow-xl border border-gray-200 py-1 min-w-[140px] z-50">
              <button onClick={() => applyHeading('h1')} className="w-full px-4 py-2 text-sm text-left hover:bg-gray-50 transition-colors">
                大見出し
              </button>
              <button onClick={() => applyHeading('h2')} className="w-full px-4 py-2 text-sm text-left hover:bg-gray-50 transition-colors">
                小見出し
              </button>
              <button onClick={() => applyHeading('p')} className="w-full px-4 py-2 text-sm text-left hover:bg-gray-50 transition-colors">
                指定なし
              </button>
            </div>
          )}
        ...
        <div className="relative alignment-menu">
          ...
          {showAlignmentMenu && (
            <div className="absolute top-full left-0 mt-1 bg-white rounded-lg shadow-xl border border-gray-200 py-1 min-w-[120px] z-50">
              <button onClick={() => applyAlignment('left')} className="w-full px-4 py-2 text-sm text-left hover:bg-gray-50 transition-colors">
                左揃え
              </button>
              <button onClick={() => applyAlignment('center')} className="w-full px-4 py-2 text-sm text-left hover:bg-gray-50 transition-colors">
                中央揃え
              </button>
              <button onClick={() => applyAlignment('right')} className="w-full px-4 py-2 text-sm text-left hover:bg-gray-50 transition-colors">
                右揃え
              </button>
            </div>
          )}
          ...
          onClick={(e) => {
            ...
            if (selection && selection.rangeCount > 0) {
              ...
              if (!range.collapsed) {
                ...
                if (existingLink?.href) setLinkUrl(existingLink.href);
  ...
  const applyLink = () => {
    ...
    const actualBlockId = saved.blockId.endsWith('-caption') ? saved.blockId.replace('-caption', '') : saved.blockId;
  ...
  const deleteListItem = (blockId: string, itemIndex: number) => {
    setBlocks(
      blocks.map((b) =>
        b.id === blockId ? { ...b, listItems: (b.listItems || []).filter((_, idx) => idx !== itemIndex) } : b
      )
    );
  };

  const blocksToContent = () => JSON.stringify(blocks);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isNew) {
        ...
        const { error } = await supabase.from('blog_articles').update(articleData).eq('id', articleId);
        if (error) throw error;
        alert('記事を更新しました');
      }
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, blockId: string) => {
    ...
    try {
      ...
      updateBlock(blockId, { imageUrl: uploadedImageUrl });
    } catch (err: any) {
  ...
  const BlockMenu = ({ blockId, onClose }: { blockId: string; onClose: () => void }) => (
    <div className="absolute left-0 top-full mt-2 bg-white rounded-lg shadow-xl border border-gray-100 z-50 w-64 overflow-hidden animate-fade-in-up">
      <div className="p-2 grid grid-cols-1 gap-0.5 max-h-[400px] overflow-y-auto">
        ...
        <button onClick={() => { addBlock(blockId, 'paragraph'); onClose(); }} className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"><IconFileText className="w-4 h-4" /><span>文章</span></button>
        <button onClick={() => alert('AIアシスタント機能は準備中です')} className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"><IconSparkles className="w-4 h-4" /><span>AIアシスタント</span></button>
        <button onClick={() => { addBlock(blockId, 'image'); onClose(); }} className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"><IconImage className="w-4 h-4" /><span>画像</span></button>
        <button onClick={() => alert('音声機能は準備中です')} className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"><IconMic className="w-4 h-4" /><span>音声</span></button>
        <button
          onClick={async () => {
            const url = prompt('埋め込みたいURLを入力してください:');
            if (url && url.trim()) {
              const newBlockId = generateId();
              const tempBlock: Block = { id: newBlockId, type: 'embed', content: url.trim(), embedData: { url: url.trim() } };
              ...
              try {
                ...
                setBlocks(newBlocks.map((b) => (b.id === newBlockId ? { ...b, embedData: embedData || { url: url.trim() } } : b)));
              } catch {
                setBlocks(newBlocks.map((b) => (b.id === newBlockId ? { ...b, embedData: { url: url.trim() } } : b)));
              }
            }
          }}
        ...
        <button onClick={() => alert('ファイル機能は準備中です')} className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"><IconPaperclip className="w-4 h-4" /><span>ファイル</span></button>
        <button onClick={() => alert('目次機能は準備中です')} className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"><IconTable className="w-4 h-4" /><span>目次</span></button>
        <button onClick={() => { addBlock(blockId, 'heading1'); onClose(); }} className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"><IconHash className="w-4 h-4" /><span>大見出し</span></button>
        <button onClick={() => { addBlock(blockId, 'heading2'); onClose(); }} className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"><IconHash className="w-4 h-4" /><span>小見出し</span></button>
        <button onClick={() => { addBlock(blockId, 'bulletList'); onClose(); }} className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"><IconList className="w-4 h-4" /><span>箇条書きリスト</span></button>
        <button onClick={() => { addBlock(blockId, 'numberedList'); onClose(); }} className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"><IconList className="w-4 h-4" /><span>番号付きリスト</span></button>
        <button onClick={() => { addBlock(blockId, 'quote'); onClose(); }} className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"><IconQuote className="w-4 h-4" /><span>引用</span></button>
        <button onClick={() => { addBlock(blockId, 'code'); onClose(); }} className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"><IconCode className="w-4 h-4" /><span>コード</span></button>
        <button onClick={() => { addBlock(blockId, 'divider'); onClose(); }} className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"><IconMinus className="w-4 h-4" /><span>区切り線</span></button>
        <button onClick={() => alert('有料エリア指定は準備中です')} className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"><IconDollarSign className="w-4 h-4" /><span>有料エリア指定</span></button>
      </div>
    </div>
  );
        ...
        return (
          <div className="group relative">
              ...
              <div className="relative heading-alignment-menu" onClick={(e) => e.stopPropagation()}>
                ...
                {showHeadingAlignmentMenu === block.id && (
                  <div className="absolute top-full left-0 mt-1 bg-white rounded-lg shadow-xl border border-gray-200 py-1 min-w-[120px] z-50" onClick={(e) => e.stopPropagation()}>
                    <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); updateBlock(block.id, { textAlign: 'left' }); setShowHeadingAlignmentMenu(null); }} className={`w-full px-4 py-2 text-sm text-left hover:bg-gray-50 transition-colors ${(block.textAlign || 'left') === 'left' ? 'bg-gray-50' : ''}`}>左揃え</button>
                    <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); updateBlock(block.id, { textAlign: 'center' }); setShowHeadingAlignmentMenu(null); }} className={`w-full px-4 py-2 text-sm text-left hover:bg-gray-50 transition-colors ${block.textAlign === 'center' ? 'bg-gray-50' : ''}`}>中央揃え</button>
                    <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); updateBlock(block.id, { textAlign: 'right' }); setShowHeadingAlignmentMenu(null); }} className={`w-full px-4 py-2 text-sm text-left hover:bg-gray-50 transition-colors ${block.textAlign === 'right' ? 'bg-gray-50' : ''}`}>右揃え</button>
                  </div>
                )}
              </div>
            </div>
            <input
              ...
              placeholder={block.type === 'heading1' ? '大見出し' : '小見出し'}
              className={`w-full ${block.type === 'heading1' ? 'text-3xl font-bold' : 'text-2xl font-semibold'} border-none outline-none bg-transparent ${block.textAlign === 'center' ? 'text-center' : block.textAlign === 'right' ? 'text-right' : 'text-left'}`}
            />
          </div>
        );
      case 'image':
        return (
          <div className="space-y-2">
            ...
            />
            {!block.imageUrl ? (
              <div className="border-2 border-dashed border-gray-200 rounded-lg p-8 text-center hover:bg-gray-50 transition-colors cursor-pointer group" onClick={() => {
                ...
                <button type="button" onClick={(e) => { e.stopPropagation(); const url = prompt('画像URLを入力してください'); if (url) updateBlock(block.id, { imageUrl: url }); }} className="mt-2 text-xs text-gray-300 hover:text-blue-500 underline">
                  またはURLを入力
                </button>
              </div>
            ) : (
              <div className="relative group">
                <img src={block.imageUrl} alt="" className="max-w-full h-auto rounded-lg border border-gray-200" />
                <button type="button" onClick={() => updateBlock(block.id, { imageUrl: '' })} className="absolute top-2 right-2 bg-black/50 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <div className="relative">
              <div
                ...
                style={{ minHeight: '24px', wordBreak: 'break-word', userSelect: 'text', WebkitUserSelect: 'text' }}
              />
              <style>{`
                div[data-placeholder]:empty:before { content: attr(data-placeholder); color: #9ca3af; pointer-events: none; }
                [contenteditable] { -webkit-user-select: text !important; -moz-user-select: text !important; -ms-user-select: text !important; user-select: text !important; }
                [contenteditable] * { -webkit-user-select: text !important; -moz-user-select: text !important; -ms-user-select: text !important; user-select: text !important; }
              `}</style>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {(block.listItems || ['']).map((item, idx) => (
              <div key={idx} className="flex items-center gap-2">
                <span className="text-gray-400 w-6">{block.type === 'bulletList' ? '•' : `${idx + 1}.`}</span>
                <input type="text" value={item} onChange={(e) => updateListItem(block.id, idx, e.target.value)} placeholder="リスト項目" className="flex-1 border-none outline-none bg-transparent" />
                {(block.listItems?.length || 0) > 1 && (
                  <button type="button" onClick={() => deleteListItem(block.id, idx)} className="text-gray-400 hover:text-red-500">
                    <IconX className="w-4 h-4" />
                  </button>
                )}
              </div>
            ))}
            <button type="button" onClick={() => addListItem(block.id)} className="text-sm text-gray-500 hover:text-gray-700 ml-8">
              + 項目を追加
            </button>
          </div>
        );
      case 'quote':
        return <div className="border-l-4 border-gray-300 pl-4 italic"><textarea value={block.content} onChange={(e) => updateBlock(block.id, { content: e.target.value })} placeholder="引用文" rows={3} className="w-full border-none outline-none bg-transparent resize-none" /></div>;
      case 'code':
        return <div className="bg-gray-800 text-gray-100 rounded-lg p-4 font-mono text-sm"><textarea value={block.content} onChange={(e) => updateBlock(block.id, { content: e.target.value })} placeholder="コードを入力" rows={3} className="w-full bg-transparent border-none outline-none resize-none placeholder-gray-500" /></div>;
      case 'divider':
        return <div className="py-4"><hr className="border-t border-gray-200" /></div>;
      case 'embed': {
            ...
            {block.embedData && (block.embedData.title || block.embedData.siteName || block.embedData.url) ? (
              <div className="space-y-3">
                {block.embedData.image && (
                  <img src={block.embedData.image} alt={block.embedData.title || '埋め込み画像'} className="w-full h-48 object-cover rounded-md" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                )}
                <div>
                  {block.embedData.siteName && <p className="text-xs text-gray-500 mb-1 uppercase tracking-wide font-medium">{block.embedData.siteName}</p>}
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">{block.embedData.title || block.content || normalizedEmbedUrl}</h3>
                  {block.embedData.description && <p className="text-sm text-gray-600 line-clamp-2 mb-2">{block.embedData.description}</p>}
                  <a href={normalizedEmbedUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 hover:text-blue-800 mt-2 inline-block break-all">
                    {normalizedEmbedUrl}
                  </a>
                </div>
                <button type="button" onClick={async () => {
                  ...
                  if (newUrl && newUrl.trim() && newUrl !== (block.content || normalizedEmbedUrl)) {
                    ...
                    updateBlock(block.id, { content: newUrl.trim(), embedData: embedData || { url: newUrl.trim() } });
                  }
              ...
              <div className="text-center py-4">
                <p className="text-sm text-gray-500 mb-2">{embedUrl ? 'OGP情報を読み込み中...' : '埋め込みURLがありません'}</p>
                {embedUrl && <p className="text-xs text-gray-400 mb-3 break-all">{normalizedEmbedUrl}</p>}
                <button type="button" onClick={async () => {
                  if (!embedUrl) {
                    const url = prompt('URLを入力してください:');
                    if (url && url.trim()) {
                      const embedData = await fetchOGPData(url.trim());
                      updateBlock(block.id, { content: url.trim(), embedData: embedData || { url: url.trim() } });
                    }
                  } else {
                    const embedData = await fetchOGPData(embedUrl);
                    updateBlock(block.id, { embedData: embedData || { url: embedUrl } });
                  }
        ...
        return (
          <div className="relative">
            <div
              ...
              style={{ minHeight: '120px', userSelect: 'text', WebkitUserSelect: 'text' }}
    ...
    <div className="p-6 md:p-8 bg-gray-50 min-h-screen">
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button
                ...
                className={`px-4 py-2 bg-primary text-white rounded-md transition-colors ${!linkUrl.trim() ? 'opacity-50 cursor-not-allowed' : 'hover:bg-primary/90'}`}
              >
                適用
              </button>
            </div>
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900">{isNew ? '新規記事作成' : '記事編集'}</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex items-start gap-4">
                    <div className="flex-shrink-0 w-8 pt-2">
                      ...
                      {showBlockMenu === block.id && (
                        <div className="relative">
                          <BlockMenu blockId={block.id} onClose={() => setShowBlockMenu(null)} />
                        </div>
                      )}
                    </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">抜粋（記事一覧に表示される短い説明）</label>
              <textarea
                value={excerpt}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">メイン画像URL（記事一覧のサムネイル）</label>
              <input
                type="url"
                value={imageUrl}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">note URL（オプション）</label>
              <input
                type="url"
                value={noteUrl}
