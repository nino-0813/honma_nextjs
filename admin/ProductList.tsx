...
const ProductList = () => {
      ...
      const productsWithOrder = sortedData.map((product: any, index) => ({
        ...
        display_order:
          product.display_order !== null && product.display_order !== undefined
            ...
            : index,
        is_visible:
          product.is_visible !== null && product.is_visible !== undefined
            ? product.is_visible
            : true,
      }));

      setProducts(productsWithOrder as ProductRow[]);
    } catch (error) {
  ...
  const calculateVariantStock = (product: ProductRow): number => {
    ...
    for (const vt of variantsConfig) {
      if (vt.stockManagement === 'none') continue;
      if (vt.stockManagement === 'individual' || vt.stockManagement === 'shared') {
        let typeStock = 0;
        ...
        if (typeStock > 0) variantStocks.push(typeStock);
      }
    }

    if (variantStocks.length === 0) return 0;
    if (variantStocks.length === 1) return variantStocks[0];
  ...
  const handleDragOver = (e: React.DragEvent, productId: string) => {
    ...
    if (draggedItem && draggedItem !== productId) setDragOverItem(productId);
  };
  ...
  const handleDrop = (e: React.DragEvent, targetProductId: string) => {
    ...
    if (!isReorderMode || !draggedItem || draggedItem === targetProductId) {
      setDraggedItem(null);
      return;
    }
    ...
    for (const filteredProduct of newFilteredProducts) {
      updates.push({ id: filteredProduct.id, display_order: currentOrder++ });
    }
    for (const product of sortedProducts) {
      if (!filteredProductIds.includes(product.id)) {
        updates.push({ id: product.id, display_order: currentOrder++ });
      }
    }

    const updatedProducts = updates
      .map((u) => {
    ...
    try {
      for (let idx = 0; idx < ordered.length; idx++) {
        ...
        const { error } = await supabase
          ...
          .eq('id', item.id);
        if (error) throw new Error(error.message || '更新エラー');
      }
      await fetchProducts();
      setIsReorderMode(false);
      setHasUnsavedReorder(false);
      ...
      try {
        ...
        for (const product of updates) {
          try {
            ...
            const existingHasVariants = Boolean(existing && (((existing as any).has_variants) || existingHasVariantsFromConfig));
            if (existingHasVariants) product.stock = 0;

            if (existing) {
              const { error: updateError } = await client.from('products').update(product).eq('id', existing.id);
              if (updateError) throw updateError;
            } else {
              if (!product.handle) {
                product.handle = product.sku.toLowerCase().replace(/[^a-z0-9]+/g, '-') || `prod-${Date.now()}`;
              }
              const { data: existingHandle } = await client
                .from('products')
                .select('id')
                .eq('handle', product.handle)
                .maybeSingle();

              if (existingHandle) product.handle = `${product.handle}-${Date.now()}`;
              const { error: insertError } = await client.from('products').insert(product);
              if (insertError) throw insertError;
            }
            successCount++;
          } catch (error: any) {
            failureCount++;
            failureDetails.push(`SKU: ${product.sku} (${product.title || 'タイトル不明'}) - ${error?.message || '不明なエラー'}`);
          }
        }
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Link
            ...
            className="bg-black text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-gray-800 transition-all shadow-sm hover:shadow flex items-center gap-2"
          >
                ...
                {filteredProducts.map((product) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-gray-500">
                      {product.category}
                      {product.subcategory && (
                        <span className="text-xs text-gray-400 ml-1">({product.subcategory})</span>
                      )}
                    </td>
