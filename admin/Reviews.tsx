...
const Reviews = () => {
  ...
  const uploadImageToStorage = async (file: File): Promise<string> => {
    ...
    const { error: uploadError } = await supabase.storage.from('product-images').upload(filePath, webpFile, {
                    ...
                    {formData.images.map((imageUrl, index) => (
                      <div key={index} className="relative group">
                        <div className="aspect-square bg-gray-100 rounded-lg overflow-hidden border border-gray-200">
                          <img src={imageUrl} alt={`レビュー画像 ${index + 1}`} className="w-full h-full object-cover" />
                        </div>
                        <button
                          type="button"
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      {review.role && <span className="text-xs text-gray-500">({review.role})</span>}
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {review.product_name && <span className="text-xs text-gray-500">・{review.product_name}</span>}
                    </div>
