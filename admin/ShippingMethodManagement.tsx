...
import type { ShippingMethod, AreaFees } from '@/types';
...
const ShippingMethodManagement = () => {
  ...
  const fetchShippingMethods = async () => {
    ...
    try {
      ...
      const methods = (data || []).map((method: Record<string, unknown>) => ({
        ...method,
  ...
  const formatAreaFees = (method: ShippingMethod): string => {
    if (method.fee_type === 'uniform') {
      return method.uniform_fee != null ? `全国一律: ¥${method.uniform_fee.toLocaleString()}` : '未設定';
    }

    const fees = Object.entries(method.area_fees || {})
      .filter(([, fee]) => fee !== null && fee !== undefined)
      .map(([area, fee]) => `${AREA_LABELS[area as keyof AreaFees]}: ¥${Number(fee).toLocaleString()}`)
      .join(', ');
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">ダンボール単位の発送方法と送料を管理します</p>
        </div>
        <Link
          ...
          {shippingMethods.map((method) => (
            ...
            >
              ...
              <div className="mb-4 pt-4 border-t border-gray-100">
                ...
                <div className="text-sm text-gray-900 font-medium">{formatAreaFees(method)}</div>
              </div>
