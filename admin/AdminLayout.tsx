...
const AdminLayout: React.FC<AdminLayoutProps> = ({ children, title, actions }) => {
  ...
  const NavItem = ({ href, icon: Icon, label }: { href: string; icon: any; label: string }) => {
    ...
    return (
      <Link href={href} className={`flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 ${
        isActive 
    ...
    <div className="min-h-screen bg-gray-50 font-sans text-primary flex">
      ...
      <aside className="w-64 bg-white border-r border-gray-100 fixed h-full z-20 hidden md:flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          <NavItem href="/admin/start-set" icon={IconPackage} label="スタートセット設定" />
          ...
          <NavItem href="/admin/referrals" icon={IconTrendingUp} label="紹介実績" />
