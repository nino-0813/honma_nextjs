  ...
  const fetchCustomers = async () => {
    ...
    try {
      ...
      if (ordersResult.error) {
        console.warn('注文データ取得エラー:', ordersResult.error);
      }
      ...
      profiles.forEach((profile: any) => {
        ...
        customerMap.set(profile.email, {
          ...
          lastOrderDate: userOrders.length > 0 ? userOrders[0].created_at : profile.created_at,
                ...
                {filteredCustomers.map((customer) => (
                  <tr key={customer.id} className="group hover:bg-gray-50/80 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-2.5 py-1 rounded-md text-xs font-medium ${
                          customer.hasOrders ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'
                        }`}
                      >
                    ...
                    <td className="px-6 py-4 text-gray-500">
                      {customer.lastOrderDate ? new Date(customer.lastOrderDate).toLocaleDateString('ja-JP') : '-'}
                    </td>
                  </tr>
                ))}
              </tbody>
