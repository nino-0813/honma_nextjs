    ...
    <>
      ...
      {open && (
        <div role="dialog" aria-modal="true" aria-label="イケベジ品種比較表" className="fixed inset-0 z-[100] flex items-center justify-center bg-black/65 p-3 md:p-8" onClick={() => setOpen(false)}>
          <div className="relative h-[88vh] w-full max-w-6xl overflow-hidden bg-white" onClick={(event) => event.stopPropagation()}>
            <button type="button" aria-label="比較表を閉じる" onClick={() => setOpen(false)} className="absolute right-3 top-3 z-10 flex h-11 w-11 items-center justify-center rounded-full bg-white text-2xl shadow">×</button>
            <iframe title="イケベジ品種比較表" src="/docs/ikevege-rice-selection-guide.pdf" className="h-full w-full" />
          </div>
        </div>
      )}
    </>
